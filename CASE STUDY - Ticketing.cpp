...
void sortByDate(){
	...
	char customerName[10], eventType[100], eventName[100];
	int ticketTotal, ticketPrice, total, hari, bulan, tahun;
	while (fscanf(fp, "\n%d/%d/%d %s %s %s %d %d %d \n", &hari, &bulan, &tahun, customerName, eventType, eventName, &ticketTotal, &ticketPrice, &total) != EOF){
		strcpy(data[count].customerName, customerName);
		strcpy(data[count].eventType, eventType);
		strcpy(data[count].eventName, eventName);
		data[count].ticketTotal = ticketTotal;
...
void sortByTicket(){
	...
	FILE *fp = fopen("sales.txt", "r");
	count = 0;
	char customerName[100], eventType[100], eventName[100];
	int ticketTotal, ticketPrice, total, hari, bulan, tahun;
	while (fscanf(fp, "\n%d/%d/%d %s %s %s %d %d %d\n", &hari, &bulan, &tahun, customerName, eventType, eventName, &ticketTotal, &ticketPrice, &total) != EOF){
		strcpy(data[count].customerName, customerName);
		strcpy(data[count].eventType, eventType);
	...
	for (int i = 0; i < T; i++){
        for (int j = 0; j < T - i; j++){
            if (data[j].ticketTotal < data[j + 1].ticketTotal){
                ticket temp = data[j];
                data[j] = data[j + 1];
                data[j + 1] = temp;
            }
